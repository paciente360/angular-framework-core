	...
	var directive = function ($compile, $document, $window) {
		return {
			...
			link: function (scope, element, attrs, ngModel) {
				...
				// set colors
				if (scope.setColors) {
					scope.setColors = JSON.parse(scope.setColors);
					scope.setColors.forEach(function (color) {
						if (newRow == 0) {
							template += '<div class="color-picker-row-wrapper">' +
								'<div class="color-picker-row">' + '<cp-color class="color-picker ' + color + '"color="' + colors[color] + '">' +
								'</cp-color>';
							newRow++;
						} else if (newRow == 3) {
							template += '<cp-color class="color-picker ' + color + '"color="' + colors[color] + '">' +
								'</cp-color>' + '</div>' + '</div>';
							newRow = 0;
						} else {
							template += '<cp-color class="color-picker ' + color + '"color="' + colors[color] + '">' +
